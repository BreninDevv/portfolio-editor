    ...
    <main className="bg-[#121212] font-sans selection:bg-white selection:text-black min-h-screen">
      ...
      >
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-white uppercase text-xs md:text-sm tracking-tighter">
              YouBrenno
            </span>
          </div>
          <nav className="flex gap-4 md:gap-6 font-black text-white/50 uppercase text-[10px] md:text-xs tracking-widest">
      ...
      <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden px-4">
        ...
        <div className="relative z-10 text-center flex flex-col items-center w-full">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-black text-white uppercase tracking-tighter mb-2 leading-none">
            YOUBRENNO
          </h1>
          <p className="text-white/40 font-bold uppercase tracking-[0.2em] md:tracking-[0.3em] text-[10px] sm:text-xs md:text-sm">
            Professional Dark Content Editor & Specialist
          </p>
        </div>
      </section>
      ...
      >
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-white/30 font-black uppercase tracking-[0.3em] text-xs">
            Available for Dark YouTube Channels
          </p>
        </div>

        <div className="w-full max-w-6xl flex flex-col md:flex-row items-center justify-center gap-10 md:gap-16">
          <div className="hidden md:block w-full border-4 border-white/10 rounded-[2.5rem] md:rounded-[3rem] overflow-hidden max-w-[400px] transition-transform hover:scale-[1.02] duration-500 shadow-[20px_20px_0px_0px_rgba(255,255,255,0.05)]">
            <Image
              src={Personagem2}
              alt="Breno"
              width={600}
...
function VideoCard({ item }: { item: any }) {
  ...
  return (
    <div className="group flex flex-col gap-4">
      <div
        className={`relative w-full bg-[#181922] border-[3px] md:border-4 border-[#181922] rounded-[2rem] md:rounded-[2.5rem] overflow-hidden shadow-[8px_8px_0px_0px_#181922] md:shadow-[12px_12px_0px_0px_#181922] transition-all group-hover:translate-x-1 group-hover:translate-y-1 group-hover:shadow-none ${item.type === "vertical" ? "aspect-[9/16]" : "aspect-video"}`}
      >
        {embedUrl ? (
          <iframe
          ...
          <div className="w-full h-full flex items-center justify-center bg-[#181922]">
            <span className="text-white/20 font-black uppercase text-lg italic text-center px-4">
              {item.title}
            </span>
          </div>
        )}
